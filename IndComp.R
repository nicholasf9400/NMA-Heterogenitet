IndComb <- function(net, t1, t2, effect){
  ...
  if(log_sm){
    ...
    tot.ind <- data.frame('Total Indrect Effect (95% CI)','','', paste0(round(ind.TE, 2), ' (',ind.TE-1.96*ind.seTE,ind.TE+1.96*ind.seTE, ')'), ind.seTE
                          ,ind.TE-1.96*ind.seTE,ind.TE+1.96*ind.seTE, ind.TE, ' ')
  }

  # Name output for compiling results
  colnames(out) <- c('Treatment 1', "via", 'Treatment 2', paste0(net$sm, ' (95% CI)'), 'se', 'lower', 'upper', 'est', "I2")
