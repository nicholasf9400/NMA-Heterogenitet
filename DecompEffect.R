...
DecompEffect <- function(net, t1, t2, effect){
  ...
  if (prop.dir[comp] == 1) {
    ...
    min.scale <- min(as.numeric(dir.effect$lower), na.rm = T)
    max.scale <- max(as.numeric(dir.effect$upper), na.rm = T)
  ...
  if (prop.dir[comp] == 0) {
    ...
    # Scale of forest plot
    min.scale <- min(as.numeric(ind.effect$lower), na.rm = T)
    max.scale <- max(as.numeric(ind.effect$upper), na.rm = T)
  ...
  if ((prop.dir[comp] != 0) & (prop.dir[comp] != 1)) {
